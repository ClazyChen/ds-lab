...
#bluetxt[实验`VectorConcat.cpp`。]在@vec:insert-continuous 中讨论的是连续插入元素情况，实际上不应该存在。一方面，这种情况不应该使用向量的自动扩容策略，而应该预先调用`reserve`函数进行预分配。这样可以避免不必要的扩容操作，提高效率。另一方面，因为要插入的不是单个元素，而是多个元素，插入操作的处理方式也可以有所不同。本节讨论一次性插入多个元素的问题，亦即向量合并问题。假设我们有两个向量`V[0:n]`和`W[0:m]`，我们希望将整个`W`插入到`V`的位置`r`处，实现向量合并。
...
#bluetxt[实验`VectorRemove.cpp`。]在 @vec:remove ，我们讨论的删除是按位置删除，具体到向量就是按_秩_删除。另一种删除的方式是按_值_删除，也就是说，我们给定一个元素`e`，想要删除向量中和`e`相等的每一个元素。
...
#h(2em)在示例的测试程序中，我们构造了长度为`n`的向量，将其随机赋值为某个区间的数并排序，然后重复$10^6$次查找（这是因为单次查找的效率太高，无法正确反映各个算法的性能差异）。我们会发现，由于进行了很多抽象，使用模板的方法性能会显著低于直接递归或迭代的性能；而递归和迭代之间相差无几（编译器对递归的版本进行了自动优化）。但即使成模板，它作为对数复杂度的算法，效率仍然远远高于在\ref{seg:查找一个元素}节中讨论的顺序查找，\textit{您可以自己实现一个基于顺序查找的算法参与对比}。
...
+ #hard 假设一个$m times n$的矩阵的每行和每列都是有序的。给定一个元素`x`，证明：为了判定`x`是否存在于该矩阵中，最坏情况下最少需要$m at n$（定义见 @vec:merge-sort 的第5题）次比较。// 移动到有序查找那里
