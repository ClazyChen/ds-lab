...
这个实验表明，当在尾部连续插入元素时，向量的效率显著高于列表，但是单向列表并没有和双向列表之间有很大的差异。根据您的运行环境不同，向量和列表大致呈现一个数倍的性能差距，总体来说是常数的。需要特别指出的是，尽管在尾部_连续_插入元素的测试中向量的性能高于列表，但对于_单次_插入元素的过程则不一定。触发扩容时，向量单次插入元素的时间复杂度可达$Theta(n)$，而列表是稳定的$O(1)$。这一点是列表的独特优势，在对响应时间稳定性有要求的场合下，向量的扩容变得不能接受，而列表的插入时间是可以保证很低的。
...
#graybox[【C++学习】#linebreak()#h(2em)
因为向量的耗时增长很快，我们在$n=10^5$量级已经可以看出它的性能，因此不需要花费大量的时间等待它在更大的规模上完成实验。这里采用了C++17引入的`if constexpr`语法，从而实现在编译期判断。上面的代码对于以向量为参数的模板实例，会将`if constexpr`内的判断编译进去；而对于以列表为参数的模板实例，则不会将`if constexpr`内的判断编译进去；这个向量和列表的区分是编译期完成的，运行期不会再做判断。这一语法简洁清晰，可以用来替代C语言风格很容易被滥用的`#ifdef`等条件编译命令。
]
...
+ #easy 写出采用裸指针的单向列表的后插算法。
+ #easy 写出采用裸指针的单向列表的前插算法。
+ #easy 写出采用裸指针的单向列表的删除算法。
...
+ #hard 如果没有哨兵节点，是否仍然可以使用尾部减治策略，在单向列表上进行倒置操作？请给出一个算法。
...
#h(2em) 在 @lis:circular-list-structure 中：#linebreak()
