...
本书采用和Java相似的命名规则。头文件后缀名采用`.hpp`，每个公共类的实现放在与其同名的`.hpp`文件中，同一个命名空间的所有类放在同一个目录下，目录的路径为命名空间的名称。比如，上面的文件位于`dslab/framework/Object.hpp`，其中`dslab`为母空间，`framework`为子空间，`Object`为类名。本章《绪论》里的所有头文件都会位于`dslab/framework`目录下，类似地，本章的实验都会位于`lab/framework`目录下。介绍实验时会用#bluetxt[蓝色黑体字]高亮标注其所在文件，读者可以按照自己的习惯进行实验。
...
#graybox[
【C++学习】#linebreak()
#h(2em)#bluetxt[实验`voa.cpp`。]C++标准库提供的一些*容器*模板，实际上是一些数据结构的封装。这些容器模板的第一个模板参数通常就是数据结构中的元素类型，比如`std::vector<int>`表示元素是`int`类型的一个向量（见《向量》章）。如果用户希望表示任意类型的元素组成的向量，可以使用`std::vector<std::any>`。在访问一个具体元素的时候，需要使用`std::any_cast<T>`来将其转换成实际的元素类型`T`。这种方法相比于C语言的`void*`具有类型安全的优点。
...
#graybox[
#h(2em)
在C++标准库中还存在另外两个函数可以用来求和，分别在C++17和C++23引入。这三种函数都可以用于任意的归约（如使用乘、异或等代替求和中的加），不同编译器对于这三种函数的优化程度不同，其中一些编译器有可能应用了SIMD技术来提高执行效率，这大约会带来数倍的时间差距。
```cpp
...
#graybox[#h(2em)
    本书中的表格和图像都是通过实验得到的（GCC 13.2.0；单位为毫秒），其他编译器可能会使得实验结果有所不同，尤其是@tab:sum2 中没有展示的、使用标准库函数的三列。请读者在自己的环境中进行实验，以得到更加准确的结果，并和本书中的结果对比分析。
]
...
在《累加问题》节中：#linebreak()
+ SumBasic的时间复杂度是多少？
+ SumAP和SumAP2这两种实现，各自在$n$处于什么区间时可以输出正确的值？#linebreak()
+ 如果它们的输入输出范围不是32位的`int`，而是`W`位的带符号整型，则这两种实现各自在$n$处于什么区间时可以输出正确的值？并分析`W` $ arrow.r infinity$时的情况。

#h(2em)
在《最大公因数》节中：#linebreak()
+ 分析朴素的最大公因数算法GcdBasic（见`gcd.cpp`）和欧几里得算法的时间复杂度和空间复杂度。
+ 假设$m$位整数的乘法和带余数除法时间复杂度为$O(m log m)$@knuth1997artv2 @david2021multiplication，分析在输入数据的位宽为$m$时，朴素的最大公因数算法和欧几里得算法的时间复杂度和空间复杂度。
+ 对中国古典名著《九章算术》里介绍的中华更相减损术@邓俊辉2013数据结构 GcdCN（见`gcd.cpp`）做正确性检验，并分析其在`int`和$m$位整数两种情形下的时间复杂度。请注意除以2的操作在计算机中只需要移位就可以完成，因此其时间复杂度和移位一致，是$O(m)$而非$O(m log m)$。
+ 将中华更相减损术转换为递归算法，并分析其空间复杂度。

#h(2em)
在《数组求和》节中：#linebreak()
+ 对迭代算法ArraySumIterative做尾部减治，就对应了`asum.cpp`里展示的减治算法；请实现头部减治所对应的算法。
+ 证明`asum.cpp`中的减治算法和分治算法的正确性，并分析它们的时间复杂度和空间复杂度。
+ 在`asum.cpp`中的分治算法，将数组等分成了两个部分，然后递归地求解这两个部分的和。如果将数组等分成$k$个部分（$k >= 2$是一个常数），然后递归地求解这$k$个部分的和，会得到什么样的算法？分析它的时间复杂度和空间复杂度。
+ 采用本节介绍的减治和分治方法，分别设计求一个数组最大值的算法，并分析它们的时间复杂度和空间复杂度。

#h(2em)
在《函数零点》节中：#linebreak()
+ #bluetxt[实验`mid.cpp`。]在二分算法的示例程序中将`mid`赋值为了$l+(r-l)/2$。其他常见的写法还有$(l+r)/2$和$l/2+r/2$。数学上这两者是相同的，但由于计算机中的数据有位宽的限制，会导致这三种写法的实际效果有所不同。请在整型和浮点型两种场合下，分析这三种写法的优劣。
+ 在二分算法的示例程序中，如果把$f(l)dot f("mid") <= 0$中的小于等于改为严格小于，会造成什么结果？

#h(2em)
在《复杂度记号的定义》节中：#linebreak()
+ 证明$Theta(log _a n) = Theta(log _b n)$对一切$a, b > 1$成立。正是因为这个原因，在计算机领域通常省略底数，直接写作$Theta(log n)$。
+ 证明$log n = O(n^c)$对一切$c > 0$成立。
+ 证明$n^c = O(a^n)$对一切$c > 0, a > 1$成立。
+ 在现代的通用个人计算机上，一秒大约可以完成$10^9$数量级的原子操作。假设关于该原子操作的常数为1，那么一个$Theta(n)$的算法只有在$n <= 10^9$的条件下才能在一秒内完成。类似地，请讨论$Theta(log n), Theta(n log n), Theta(n^2), Theta(n^3), Theta(2^n)$以及$Theta(n!)$的算法分别在多大的$n$下才能在一秒内完成。
+ 如果$T(n)$是等差数列，那么它的时间复杂度是什么？如果$T(n)$是等比数列，那么它的时间复杂度是什么？

#h(2em)
在《复杂度记号的常见理解误区》节中：#linebreak()
+ 在$O(log(n!)), Theta(log(n!)), Omega(log(n!)), O(n log n),Theta(n log n), Omega(n log n)$之间，写出所有可以用等号连接的符号对。
+ 正文中寻找第一个偶数的算法是从前向后遍历数组的，如果从后向前遍历数组，会得到什么样的结果？如何才能刻画这两种算法的效率差异？

#h(2em)
在《判断2的幂次》节中：#linebreak()
+ 证明IsPower2Iterative的时间复杂度是$Theta(log n)$，空间复杂度是$O(1)$；而IsPower2Recursive的时间复杂度和空间复杂度均为$O(log n)$和$Omega(1)$。
+ 将IsPower2Iterative转换为与其等效的递归算法，将IsPower2Recursive转换为与其等效的迭代算法。
+ `int`溢出问题是否会对本节介绍的几种算法的稳健性产生影响？如果会，如何改正它？
+ 如果按照“输入规模派”的观点，在判断2的幂次的问题中，问题规模应该如何定义？并在这个定义下，分析IsPower2Basic、IsPower2Recursive和IsPower2Iterative的时间复杂度和空间复杂度。
+ 判断2的幂次等价于判断一个正整数的二进制表示是否只有一个1。现在我们希望输出一个正整数的二进制表示中1的数量，在不借用`<bit>`库的情况下，分别利用迭代和递归设计相应的解决方案，并分析它们的时间复杂度和空间复杂度（问题规模采用输入规模定义）。 // 和矩阵乘法联系起来
+ 如果希望算法输出的是输入的正整数“按位颠倒”的结果（包括前导0但不包括符号位，如原数据为00000001 11011001 10011111 11000001；则按位颠倒之后的结果为01000001 11111100 11001101 11000000），又应该如何设计算法呢？

#h(2em)
在《快速幂》节中：#linebreak()
+ 根据正文中的递推式，将PowerBasic转换为对应的减治递归形式。从这个快速幂的例子中也可以看出分治算法相对减治的好处。
+ 将PowerFastRecursive转换为对应的迭代形式。
+ 借助$a^b = (a^(b/2))^2$，写出另一种快速幂的实现。// 和归并排序联系起来

#h(2em)
在《复杂度的积分计算》和《多重循环复杂度的简单估算》节中：#linebreak()
分析以下程序的时间复杂度（不考虑编译器优化）。定义最内层循环每次执行的时间是1个单位的情况下，估算这些时间复杂度的常数。
+ ```cpp
void F1(int n) {
    for (int i { 0 }; i < n; ++i)
    for (int j { 0 }; j < n; ++j);
