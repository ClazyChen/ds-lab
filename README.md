...
第二章：**向量**。扩容缩容，批量删除，置乱，归并排序，折半查找，循环位移。

第三章：**列表**。单向、双向的列表，动态、静态的列表；归并排序。

第四章：**栈**。括号匹配，尾递归改写，中缀表达式计算，中缀转后缀；共享栈。随机生成栈混洗。进制转换。

第五章：**队列**。链式队列、循环队列、基于向量的队列。（受限的）双端队列。

第六章：**矩阵**。稀疏矩阵的压缩存储，三元组，十字链表。

第七章：**二叉树**。四种遍历，递归和迭代版本。线索化。中序+X重建二叉树。随机生成二叉树。Huffman编码。

第八章：**树和森林**。二叉树、树和森林的相互转化。树和森林的遍历。并查集。

第九章：**AVL树**。

第十章：**完全二叉堆**。堆排序。

第十一章：**图**。邻接矩阵、邻接表，DFS，BFS，拓扑排序。最短路径，最小生成树。AOV网，AOE网。

第十二章：**查找**。块状结构。fibonacci查找。插值查找。
...
第十六章：**串**。KMP，BM，Karp-Rabin。

第十六章：**排序**。直接插入排序，折半插入排序，希尔排序。冒泡排序，快速排序，快速划分，MOM。简单选择排序，桶排序，基数排序，锦标赛排序，置换-选择排序。
