...
1. 框架使用C++17-C++20的标准。
   ...
   > C++20的`<ranges>`库是个很有潜力的东西，但是开放给用户的不够丰富，很多工作被留到C++23去了，不得不说是一个遗憾。在这里现在还是只能用传统的迭代器模式去写。
   >
   > 智能指针是C++11引入的，这里没有使用它，这是为了降低普通用户的学习成本。框架里仍然保留了一些智能指针。
...
3. 密切结合STL。
   ...
   另一方面，我给出的实验框架下，您自己实现的数据结构也可以非常自然地嵌入到STL中。比如，您可以对自己设计的 `Vector`使用 `std::sort(std::begin(V), std::end(V))`这样的排序。
...
文件中使用的所有头文件都在`headers`文件夹中，您配置头文件目录的时候应该加入这个文件夹。

> 下面的内容是面向使用Windows系统、不懂得如何配置环境的同学的。已经自己有C++环境的同学加上我的头文件目录即可，不需要额外做什么环境配置工作。
...
> 如果您自己的电脑上安装有MinGW，请确保它可以支持选项`-std=c++2a`。

我用的版本是11.2.0，更高版本的MinGW应该也没有问题。

### Dev-C++的配置

Dev-C++是入门级的C++编辑器，被广大C++初学者使用。这个直接在腾讯软件中心之类的地方就能下载。

我在开发的过程中，所有代码都在Dev-C++上编译通过并确认有效。

> 我的文件采用的是UTF-8编码，Dev-C++中可能会将注释的中文变成乱码。发生这种情况，可以采用用记事本打开再复制进去等方法将编码转为Dev-C++使用的asci编码。
>

1. 进入Dev-C++，工具—编译选项—由文件夹添加编译设置，选择安装MinGW的文件夹。
2. 编译器：编译时增加加入命令 `-std=c++2a -fconcepts -O2`。（开启C++20和编译优化开关）一些情况下，您可能需要关闭 `-O2`，以避免编译器进行了过多的优化。
3. 目录：C++包含文件里，加入本实验框架中的 `headers`文件夹中的**每个子文件夹**。（**如果找不到头文件请确认是否做了这一步**）

应该这样就可以了，您可以编译一下 `sources`文件夹里的 `.cpp`文件查看效果。

## 计划安排

前17章为408共享，后4章为912特有。
